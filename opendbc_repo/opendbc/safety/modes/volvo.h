...
static const CanMsg VOLVO_EUCD_TX_MSGS[] = {
    ...
    {VOLVO_EUCD_FSM0,      VOLVO_MAIN_BUS, 8, .check_relay = true},
...
static void volvo_rx_hook(const CANPacket_t *to_push) {
  ...
  if (bus == VOLVO_MAIN_BUS) {
    ...
    if (addr == VOLVO_EUCD_FSM0) {
      // Signal: ACCStatus
      bool cruise_state = GET_BYTE(to_push, 2) & 0x04U;
      pcm_cruise_check(cruise_state);
    }
  }
}
...
static bool volvo_tx_hook(const CANPacket_t *to_send) {
  ...
  const LongitudinalLimits VOLVO_LONG_LIMITS = {
    .max_accel = 50,// +2.0 m/sˆ2
    .min_accel = -100, // -4.0 m/sˆ2
    .inactive_accel = 0,
  };
  ...
  // Longitudinal control
    if (addr == VOLVO_EUCD_FSM3) {
      int raw_accel = GET_BYTE(to_send, 1) - 126U;
